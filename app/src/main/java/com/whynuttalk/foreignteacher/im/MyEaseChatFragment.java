package com.whynuttalk.foreignteacher.im;
...
//        message.setAttribute(Constant.RECEIVOR_USERID, "150");
    ...
    @Override
    public void onMessageBubbleLongClick(String key, final EaseMessageAdapter adapter, final int position, final TextView textView) {
        TransApi transApi = new TransApi();
        transApi.getTransResult(key,"auto","en")
                .subscribe(new MaybeObserver<TransResult>() {
                    @Override
                    public void onSubscribe(Disposable d) {
                        compositeDisposable.add(d);
                    }
                    ...
                    @Override
                    public void onSuccess(TransResult transResult) {
                        adapter.addTrans(position,transResult.getTrans_result().get(0).getDst());
                    }

                    @Override
                    public void onError(Throwable e) {
                        Log.e("transErro",e.getMessage());
                    }
