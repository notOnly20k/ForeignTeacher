...
import com.whynuttalk.foreignteacher.api.dto.MyAuthentication
...
class UpLoadIdActivity : BaseTranslateStatusActivity() {
    ...
    private val user: User by lazy { appComponent.userHandler.getUser() }
    ...
    override fun initView() {
        ...
        when (type) {
            ...
            CERTIFICATE -> {
                tv_choose_type.visibility = View.VISIBLE
        ...
        tv_choose_type.setOnClickListener {
            WheelDialog.Builder()
                    .create()
                    .setDate(appComponent.netWork.Passtype.split(","))
    ...
    fun upLoad() {
        if (commitCheck()) {
            appComponent.netWork.applyAuthentication(user.id, certificateName, et_number.text.toString(), filePath, upLoadType)
