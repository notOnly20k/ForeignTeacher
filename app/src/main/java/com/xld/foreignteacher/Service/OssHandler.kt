...
class OssHandler(context: Context) {
    ...
    private val endpoint = "http://oss-cn-hangzhou.aliyuncs.com"
    private val stsServer = "STS应用服务器地址，例如http://abc.com"
    ...
    fun ossUpload(strPath: String) {
        ...
        val put = PutObjectRequest("", "", strPath)
        ...
        put.progressCallback = OSSProgressCallback { request, currentSize, totalSize ->
            logger.debug("PutObject", "currentSize: $currentSize totalSize: $totalSize")
        }
        val task = oss.asyncPutObject(put, object : OSSCompletedCallback<PutObjectRequest, PutObjectResult> {
            override fun onSuccess(request: PutObjectRequest, result: PutObjectResult) {
                logger.debug("UploadSuccess")
                logger.debug("ETag", result.eTag)
                logger.debug("RequestId", result.requestId)
            }
